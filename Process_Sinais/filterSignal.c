// Implementação de filtro digital ao estilo da função filter() do Matlab
// Ref.: https://rosettacode.org/wiki/Apply_a_digital_filter_(direct_form_II_transposed)#C
...
int main(int argC,char* argV[])
{
	char *str;
	if(argC<2||argC>3){
		printf("Usage : %s <name of signal data file>  [<optional output file>]\n",argV[0]);
		printf("File example: signalData.txt\n");
	}
	else{
		if(argC!=2){
